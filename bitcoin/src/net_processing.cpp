...
// ORIM: ZeroMQ for covert channel IPC
#include <zmq.h>
...
private:
    ...
    */
    ...
    // ===== ORIM: Covert Channel Infrastructure =====
    /** ZMQ context for ORIM covert channel */
    void* m_orim_zmq_context{nullptr};
    /** ZMQ REQ socket for communicating with Python script */
    void* m_orim_zmq_socket{nullptr};
    /** Mutex to protect ZMQ socket from concurrent thread access */
    std::mutex m_orim_mutex;
    /** Flag to enable/disable ORIM functionality */
    bool m_orim_enabled{false};
    /** ORIM ZMQ endpoint (e.g., "tcp://127.0.0.1:5555") */
    std::string m_orim_endpoint;
    /** ORIM ZMQ timeout in milliseconds */
    int m_orim_timeout_ms{100};

    /** Initialize ORIM ZMQ connection */
    void InitORIM();
    /** Shutdown ORIM ZMQ connection */
    void ShutdownORIM();
    /** Send inv hashes to Python and get reordered result (sender side) */
    bool ORIMReorderInv(NodeId peer_id, const std::string& inv_type, std::vector<CInv>& vInv);
    /** Notify Python of received inv hashes (receiver side) */
    void ORIMProcessReceivedInv(NodeId peer_id, const std::string& inv_type, const std::vector<CInv>& vInv);
      ...
      m_ignore_incoming_txs(ignore_incoming_txs)
{
    ...
    // ORIM: Initialize covert channel
    InitORIM();
}

PeerManagerImpl::~PeerManagerImpl()
{
    // ORIM: Cleanup covert channel
    ShutdownORIM();
}
...
void PeerManagerImpl::InitORIM()
{
    // Check if ORIM is enabled via command line argument
    m_orim_enabled = gArgs.GetBoolArg("-enableorim", false);
    if (!m_orim_enabled) {
        LogPrint(BCLog::NET, "ORIM: Covert channel disabled\n");
        return;
    }

    // Get ORIM endpoint from config (default: tcp://127.0.0.1:5555)
    m_orim_endpoint = gArgs.GetArg("-orimendpoint", "tcp://127.0.0.1:5555");
    m_orim_timeout_ms = gArgs.GetIntArg("-orimtimeout", 100);

    // Initialize ZMQ context
    m_orim_zmq_context = zmq_ctx_new();
    if (!m_orim_zmq_context) {
        LogPrintf("ORIM: Failed to create ZMQ context\n");
        m_orim_enabled = false;
        return;
    }

    // Create ZMQ REQ socket
    m_orim_zmq_socket = zmq_socket(m_orim_zmq_context, ZMQ_REQ);
    if (!m_orim_zmq_socket) {
        LogPrintf("ORIM: Failed to create ZMQ socket\n");
    ...
    // Set socket timeout
    zmq_setsockopt(m_orim_zmq_socket, ZMQ_RCVTIMEO, &m_orim_timeout_ms, sizeof(m_orim_timeout_ms));
    zmq_setsockopt(m_orim_zmq_socket, ZMQ_SNDTIMEO, &m_orim_timeout_ms, sizeof(m_orim_timeout_ms));

    // Connect to Python server
    if (zmq_connect(m_orim_zmq_socket, m_orim_endpoint.c_str()) != 0) {
        LogPrintf("ORIM: Failed to connect to %s: %s\n", m_orim_endpoint, zmq_strerror(errno));
    ...
    LogPrintf("ORIM: Covert channel initialized. Endpoint: %s, Timeout: %dms\n", 
              m_orim_endpoint, m_orim_timeout_ms);
}

void PeerManagerImpl::ShutdownORIM()
{
    if (m_orim_zmq_socket) {
        zmq_close(m_orim_zmq_socket);
        m_orim_zmq_socket = nullptr;
    ...
    LogPrint(BCLog::NET, "ORIM: Covert channel shutdown\n");
}

bool PeerManagerImpl::ORIMReorderInv(NodeId peer_id, const std::string& inv_type, std::vector<CInv>& vInv)
{
    if (!m_orim_enabled || !m_orim_zmq_socket || vInv.empty()) {
        return false; // Fallback: use original order
    }

    // Lock mutex to ensure thread-safe access to ZMQ socket
    ...
    try {
        ...
        request.pushKV("peer_id", peer_id);
        request.pushKV("inv_type", inv_type);
        request.pushKV("timestamp", GetTime());

        UniValue hashes_array(UniValue::VARR);
        for (const auto& inv : vInv) {
            hashes_array.push_back(inv.hash.ToString());
        }
        request.pushKV("hashes", hashes_array);

        std::string request_str = request.write();

        // Send request to Python
        if (zmq_send(m_orim_zmq_socket, request_str.c_str(), request_str.size(), 0) == -1) {
            LogPrint(BCLog::NET, "ORIM: Failed to send request: %s\n", zmq_strerror(errno));
            return false;
        }
        ...
        if (size == -1) {
            LogPrint(BCLog::NET, "ORIM: Failed to receive response: %s\n", zmq_strerror(errno));
            return false;
        }
        buffer[size] = '\0';
        ...
        // Parse JSON response
        UniValue response;
        if (!response.read(buffer)) {
            LogPrint(BCLog::NET, "ORIM: Failed to parse JSON response\n");
            return false;
        }

        const UniValue& status = response["status"];
        if (!status.isStr() || status.get_str() != "success") {
            const UniValue& message = response["message"];
            LogPrint(BCLog::NET, "ORIM: Python returned error: %s\n", 
                     message.isStr() ? message.get_str() : "unknown");
            return false;
        }

        // Extract reordered hashes
        const UniValue& reordered = response["reordered_hashes"];
        if (!reordered.isArray() || reordered.size() != vInv.size()) {
            LogPrint(BCLog::NET, "ORIM: Invalid reordered_hashes array\n");
            return false;
        }

        // Build map of hash -> CInv for quick lookup
        std::map<uint256, CInv> hash_map;
        for (const auto& inv : vInv) {
            hash_map[inv.hash] = inv;
        }

        // Rebuild vInv in the new order
        std::vector<CInv> reordered_vInv;
        reordered_vInv.reserve(vInv.size());
        for (size_t i = 0; i < reordered.size(); i++) {
            uint256 hash;
            hash.SetHex(reordered[i].get_str());
            auto it = hash_map.find(hash);
            if (it == hash_map.end()) {
                LogPrint(BCLog::NET, "ORIM: Reordered hash not found in original set\n");
                return false;
            }
            reordered_vInv.push_back(it->second);
        }

        vInv = std::move(reordered_vInv);
        LogPrint(BCLog::NET, "ORIM: Successfully reordered %d inv messages for peer %d\n", 
                 vInv.size(), peer_id);
        return true;

    } catch (const std::exception& e) {
        LogPrint(BCLog::NET, "ORIM: Exception in ORIMReorderInv: %s\n", e.what());
        return false;
    }
}

void PeerManagerImpl::ORIMProcessReceivedInv(NodeId peer_id, const std::string& inv_type, const std::vector<CInv>& vInv)
{
    if (!m_orim_enabled || !m_orim_zmq_socket || vInv.empty()) {
        return; // Silently skip if ORIM not enabled
    }
    ...
    try {
        ...
        // Send notification to Python (fire-and-forget, but check for errors)
        if (zmq_send(m_orim_zmq_socket, request_str.c_str(), request_str.size(), 0) == -1) {
            LogPrint(BCLog::NET, "ORIM: Failed to send receive notification: %s\n", zmq_strerror(errno));
            return;
        }
        ...
        int size = zmq_recv(m_orim_zmq_socket, buffer, sizeof(buffer) - 1, 0);
        if (size == -1) {
            LogPrint(BCLog::NET, "ORIM: Failed to receive ack: %s\n", zmq_strerror(errno));
            return;
        }
        buffer[size] = '\0';
        ...
        if (response.read(buffer)) {
            const UniValue& status = response["status"];
            if (status.isStr() && status.get_str() == "success") {
                LogPrint(BCLog::NET, "ORIM: Processed %d received inv from peer %d\n", 
                         vInv.size(), peer_id);
            }
        }

    } catch (const std::exception& e) {
        LogPrint(BCLog::NET, "ORIM: Exception in ORIMProcessReceivedInv: %s\n", e.what());
    }
}
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        // ORIM: Notify Python of received inv (extract covert data)
        if (!vInv.empty()) {
            // Determine inv type from first element
            std::string inv_type = vInv[0].IsMsgBlk() ? "block" : "tx";
...

    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            for (const uint256& hash : peer->m_blocks_for_inv_relay) {
                ...
                if (vInv.size() == MAX_INV_SZ) {
                    // ORIM: Reorder inv before sending
                    ORIMReorderInv(pto->GetId(), "block", vInv);
        ...
        if (!vInv.empty()) {
            // ORIM: Reorder inv before sending (final flush)
            ORIMReorderInv(pto->GetId(), "tx", vInv);
            m_connman.PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
        }
